...
export const createRoomType = async (req: Request, res: Response) => {
  try {
    ...
    if (!max_occupancy || max_occupancy <= 0) {
      return res
        .status(400)
        .json({ error: "Max occupancy must be at least 1 person" });
    }
    ...
    if (existingName) {
      return res
        .status(409)
        .json({ error: `A Room Type with the name '${name}' already exists` });
    }
    ...
    res
      .status(201)
      .json({ message: "Room Type created successfully", data: newType });
  } catch (error: any) {
    console.error("Create error:", error);
    res.status(500).json({
      error:
        "An error occurred while creating the Room Type. Please check your data.",
    });
  }
};
...
export const editRoomType = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Room Type updated successfully", data: updated });
  } catch (error) {
...
export const deleteRoomType = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `Room type '${exists.name}' deleted successfully` });
  } catch (error) {
    res.status(500).json({
      error:
        "Cannot delete this Room Type because it is linked to existing rooms.",
    });
  }
};

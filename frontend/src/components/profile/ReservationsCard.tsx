    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-5 border-b border-mansio-gold/10 last:border-0">
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          {r.meal_plan && (
            <p className="text-xs text-mansio-taupe mt-0.5">{r.meal_plan.name}</p>
          )}
      ...
      <div className="flex items-center gap-4 sm:flex-col sm:items-end">
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusColors[r.status] ?? 'bg-mansio-gold/10 text-mansio-taupe'}`}>
          {r.status.replace('_', ' ')}
        </span>
        {total > 0 && (
          <p className="text-sm font-medium text-mansio-espresso">€{total.toFixed(2)}</p>
        )}
      </div>
    </div>
  )

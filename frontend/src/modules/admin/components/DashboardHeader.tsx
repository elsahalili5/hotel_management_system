...
export function DashboardHeader({ title, subtitle, onMenuClick }: DashboardHeaderProps) {
  ...
  return (
    <header className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-mansio-ink/10 bg-mansio-cream">
      <div className="flex items-center gap-3">
        <button
          onClick={onMenuClick}
          className="md:hidden p-1.5 rounded-lg text-mansio-mocha hover:opacity-70 transition-opacity"
        >
          <Menu size={20} />
        </button>
        <div>
